...
- 新建项目对话框重构为双栏布局 (边界多选+搜索 / 参考项目单选+搜索)
- 项目列表失焦选中色修复 (QPalette 而非 CSS)
- 登录后自动聚焦最新项目
- 窗口默认 1100x800, 对话框 900x500
- 用户行和会话标签行统一 40px

### 管理后台

- 边界编辑文本域 150→350px, 弹窗 560→800px
- DWG 下载支持 token query 参数
- 删除 front.html 原型文件

### 预览

- 版本树点击切换预览, 缓存无数据自动从 API 拉取

## 十、今日完成 (2026-06-11 第二波)

### 客户端界面统一

- 左侧顶栏合并为 46px 单行（用户信息 + 新建 + 退出按钮）
- 中央会话栏高度 46px，底部边线
- 右侧顶部新增 46px 栏（"版本"标题 + 退出按钮）
- 三栏顶部等高对齐，统一底部边线

### 版本表格重构

- 版本列表改为 4 列（版本/操作/版本/操作），每行横排两个版本
- 列宽 25% 自动拉伸填满，取消所有滚动条
- 表格网格线（#2C2D31）与顶栏边线颜色一致
- "推CAD"按钮固定 55px、居中
...
- QLabel 静态图改 QGraphicsView，支持滚轮缩放、拖拽平移、双击重置
- 新增 PreviewView 自定义控件，translate() 实现顺滑拖拽
- 预览图渲染分辨率提升至 540×360，预览框高度 300px
